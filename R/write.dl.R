...
cat(paste(" NM=",dim(x)[3],sep=""), file=file, append=TRUE)
cat("", file=file, sep="\n", append=TRUE)
cat(paste("FORMAT = FULLMATRIX DIAGONAL PRESENT", collapse="\n"), file=file, sep="\n", append=TRUE)
#
cat(paste("ROW LABELS:", collapse="\n"), file=file, sep="\n", append=TRUE)
for(i in 1:dim(x)[1]) {
cat(paste('\"',dimnames(x)[[1]][i],'\"', sep="", collapse="\n"), file=file, sep="\n", append=TRUE)
}
#
cat(paste("COLUMN LABELS:", collapse="\n"), file=file, sep="\n", append=TRUE)
for(i in 1:dim(x)[2]) {
cat(paste('\"',dimnames(x)[[2]][i],'\"', sep="", collapse="\n"), file=file, sep="\n", append=TRUE)
}
#
cat(paste("LEVEL LABELS:", collapse="\n"), file=file, sep="\n", append=TRUE)
for(i in 1:dim(x)[3]) {
cat(paste('\"',dimnames(x)[[3]][i],'\"', sep="", collapse="\n"), file=file, sep="\n", append=TRUE)
}
#
...
#
for(j in 1:length(vec)) lst[[as.numeric(strsplit(vec[[j]][1],", ")[[1]][1])]] <- append(lst[[as.numeric(strsplit(vec[[j]][1],", ")[[1]][1])]], as.numeric(strsplit(vec[[j]][1],", ")[[1]][2]))
#
rm(vec)
#
cat(paste("dl", " n=", dim(x)[1], " format=nodelist1", sep="", collapse="\n"), file=file, sep="\n", append=TRUE)
#
cat(paste("labels: ", collapse="\n"), file=file, sep="\n", append=TRUE)
for(i in 1:dim(x)[1]) {
cat(paste('\"',dimnames(x)[[1]][i],'\" ', sep=""), file=file, sep=" ", append=TRUE)
}
cat("", file=file, sep="\n", append=TRUE)
#
cat(paste("data:", collapse="\n"), file=file, sep="\n", append=TRUE)
