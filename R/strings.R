...
if(k>3) warning("Only equations until k = 3 will be considered.")

#
if(is.na(dim(x)[3]) == TRUE) stop("Label in matrix needed")
...
###  2+ MATRICES  ###
    if (is.na(dim(x)[3]) == FALSE) {
        ...
        tmpu<-unique(tmpo)
	dpl<-duplicated(tmpo) #<=- para el output
        if(isTRUE(nrow(tmpo)!=nrow(tmpu))==TRUE) {
	ifelse(isTRUE((nrow(tmpo)-nrow(tmpu))>1)==TRUE, note<-paste("There are",nrow(tmpo)-nrow(tmpu),"repeated relations in the data that have been equated",sep=" "),
	note<-"There is 1 repeated relation in the data that has been equated" )
	}
	#
        ...
        for (q in 1:dim(x)[3]) {
            for (j in 1:dim(x)[3]) {
                tmp<-x[, , j] %*% x[, , q]; tmp<-replace(tmp,tmp>=1,1)
                for (i in dim(x)[3]:1) {
...
##  EDGE TABLE CONSTRUCTION  ##
	...
	### edgeT const CASE: 2+
        if (is.na(dim(x)[3]) == FALSE) {
            ...
            for (q in 1:dim(x)[3]) {
                for (j in 1:dim(Bx)[3]) {
                tmp<-Bx[,,j]%*%Bx[,,q]; tmp<-replace(tmp,tmp>=1,1)
                  for (i in dim(Bx)[3]:1) {
                    if (isTRUE(all.equal(tmp, Bx[, , i]) == TRUE)) 
                      s0[j, q] <- i
                  }
...
names(luq)<-lbl
## unique string matrices as vectors
unq<-data.frame(matrix(ncol=(dim(w)[1]*dim(w)[2]),nrow=0))
for (i in 1:dim(w)[3]) {
	ifelse(isTRUE(dim(w)[3]>1)==TRUE, unq[i,]<-as.vector(w[,,i]), unq<-as.vector(w))
}; rm(i)
rownames(unq)<-lbl
#
if(k>1) {
...
if(!(paste(combn(gn,2)[,i][1],combn(gn,2)[,i][2],sep="")%in%lbl)) eq2<-append(eq2, paste(combn(gn,2)[,i][1],combn(gn,2)[,i][2],sep="") )
if(!(paste(combn(gn,2)[,i][2],combn(gn,2)[,i][1],sep="")%in%lbl)) eq2<-append(eq2, paste(combn(gn,2)[,i][2],combn(gn,2)[,i][1],sep="") )
}; rm(i)
#
eq2<-unique(eq2)
#
if(length(eq2)!=0) {
dbl<-data.frame(matrix(ncol=(dim(w)[1]*dim(w)[2]),nrow=0))
for(i in 1:length(eq2)) {
	dbl[(nrow(dbl)+1),]<-as.vector(dichot(x[,,which(dimnames(x)[[3]]==strsplit(eq2[i],"")[[1]][1])]%*%x[,,which(dimnames(x)[[3]]==strsplit(eq2[i],"")[[1]][2])]))
}; rm(i)
rownames(dbl)<-eq2
#
for(i in 1:nrow(dbl)) {
#if(isTRUE(names(luq)[i]==eq2[i])==FALSE) {
if(isTRUE(eq2[i]%in%rownames(unq))==FALSE) {
	luq[[which(duplicated(rbind(dbl[i,],unq)))-1]] <- append(luq[[which(duplicated(rbind(dbl[i,],unq)))-1]],eq2[i])
}
};rm(i)
#rm(eq2,dbl)
}
...
for(j in 1:length(gn)) {
	eq3<-append(eq3,paste(strsplit(db[j],"")[[1]][1],gn[i],strsplit(db[j],"")[[1]][2],sep=""))
}; rm(j)
}; rm(i)
#
if(length(gn)>2) {
for(i in 1:ncol(combn(gn,3))) {
if(!(paste(combn(gn,3)[,i][1],combn(gn,3)[,i][2],combn(gn,3)[,i][3],sep="")%in%lbl)) eq3<-append(eq3,paste(combn(gn,3)[,i][1],combn(gn,3)[,i][2],combn(gn,3)[,i][3],sep="") )
if(!(paste(combn(gn,3)[,i][1],combn(gn,3)[,i][3],combn(gn,3)[,i][2],sep="")%in%lbl)) eq3<-append(eq3,paste(combn(gn,3)[,i][1],combn(gn,3)[,i][3],combn(gn,3)[,i][2],sep="") )
if(!(paste(combn(gn,3)[,i][2],combn(gn,3)[,i][1],combn(gn,3)[,i][3],sep="")%in%lbl)) eq3<-append(eq3,paste(combn(gn,3)[,i][2],combn(gn,3)[,i][1],combn(gn,3)[,i][3],sep="") )
if(!(paste(combn(gn,3)[,i][2],combn(gn,3)[,i][3],combn(gn,3)[,i][1],sep="")%in%lbl)) eq3<-append(eq3,paste(combn(gn,3)[,i][2],combn(gn,3)[,i][3],combn(gn,3)[,i][1],sep="") )
if(!(paste(combn(gn,3)[,i][3],combn(gn,3)[,i][1],combn(gn,3)[,i][2],sep="")%in%lbl)) eq3<-append(eq3,paste(combn(gn,3)[,i][3],combn(gn,3)[,i][1],combn(gn,3)[,i][2],sep="") )
if(!(paste(combn(gn,3)[,i][3],combn(gn,3)[,i][2],combn(gn,3)[,i][1],sep="")%in%lbl)) eq3<-append(eq3,paste(combn(gn,3)[,i][3],combn(gn,3)[,i][2],combn(gn,3)[,i][1],sep="") )
}; rm(i)
#
}
eq3<-unique(eq3)
#
if(length(eq3)!=0) {
tpl<-data.frame(matrix(ncol=(dim(w)[1]*dim(w)[2]),nrow=0))
for(i in 1:length(eq3)) {
	tpl[(nrow(tpl)+1),]<-as.vector(dichot(x[,,which(dimnames(x)[[3]]==strsplit(eq3[i],"")[[1]][1])]%*%x[,,which(dimnames(x)[[3]]==strsplit(eq3[i],"")[[1]][2])]%*%x[,,which(dimnames(x)[[3]]==strsplit(eq3[i],"")[[1]][3])] ))
}; rm(i)
#
rownames(tpl)<-eq3
for(i in 1:nrow(tpl)) {
if(isTRUE(eq3[i]%in%rownames(unq))==FALSE) {
	luq[[which(duplicated(rbind(tpl[i,],unq)))-1]] <- append(luq[[which(duplicated(rbind(tpl[i,],unq)))-1]],eq3[i])
}
};rm(i)
#rm(db,eq3,tpl)
}
...
##############################################################################
##############################################################################
## OJO aquí el problema
#for(i in 1:length(luq)) {
#if(isTRUE(is.null(luq[[i]])==FALSE)==TRUE) {
#	ifelse(isTRUE(length(luq[[i]])==1)==TRUE, luq[[i]]<-NULL, NA)
#} else { NA }
#}; rm(i)
##############################################################################
##############################################################################
...
lqu <- paste("No equations were produced in words with length",k,"or less",sep=" ")
}
#
}
#
ifelse(equat==TRUE, lst<-list(wt = Bx, ord=dim(Bx)[3], st = lbl, equat=lqu), lst<-list(wt = Bx, ord=dim(Bx)[3], st = lbl))
    class(lst) <- "Strings"
    return(lst)
}
