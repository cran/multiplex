...
To illustrate these concepts, we apply the function \texttt{fltr()} of \textbf{multiplex} to the third element of the partial order represented by \texttt{pogc} that results in a proper principal order filter for this formal concept with labels.
...
Order ideals and principal order ideals are obtained in a similar way with this function provided that the argument \texttt{ideal} is set to \texttt{TRUE}.
...
Another possibility is to apply a force-directed layout to the binomial projection of this two-mode data set, and below the plot is made with some arguments for the vertices and the graph clock-wise rotated. 
...
\bibitem{multiplex} Ostoic, J.A.R. ~\textbf{multiplex}: \emph{Algebraic Tools for the Analysis of Multiple Social Networks}. R package v 2.9.1
\bibitem{multiplex} Ostoic, J.A.R. ~\textbf{multigraph}: \emph{Plot and manipulate multigraphs}. R package v 0.91
%
\end{thebibliography}
